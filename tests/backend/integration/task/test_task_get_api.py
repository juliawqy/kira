...
# INT-003/007
def test_list_task_filter_and_sort(client, task_base_path):
    ...
    response = client.get(f"{task_base_path}/filter?{FILTER_AND_SORT_QUERY}")
    assert response.status_code == 200
    data = response.json()
...
# INT-013/009
def test_list_parent_tasks_invalid_filter(client, task_base_path):
...
def test_manual_parents(client, task_base_path):
    resp = client.get(f"{task_base_path}/parents")
