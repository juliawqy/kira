...
from tests.mock_data.task_data import VALID_ADD, VALID_TASK_1, INVALID_TASK_NO_TITLE, INVALID_TASK_NO_STATUS, INVALID_TASK_NO_PRIORITY
...
def test_add_task_success(mock_session_local, mock_session):
    ...
    mock_task = MagicMock()
    mock_task.id = VALID_TASK_1["id"]  # Set ID = 1
    ...
    with patch("backend.src.services.task.Task", return_value=mock_task):
        result = task_service.add_task(**VALID_ADD)  # Input without ID
        assert result.id == VALID_TASK_1["id"]      # Check ID = 1
        assert result.title == VALID_TASK_1["title"] # Check same title
