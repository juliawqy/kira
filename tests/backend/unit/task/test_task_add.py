...
def test_add_task_minimal_requires_bucket_and_sets_defaults():
    """Create with minimal fields (+ required priority); verify defaults and persistence."""
    t = svc.add_task(
        title="Minimal",
        description=None,
        start_date=None,
        deadline=None,
        priority=5,  # required on creation
    )
...
def test_add_task_with_parent_links_child_to_parent():
    ...
    parent = svc.add_task(title="Parent", description=None, start_date=None, deadline=None, priority=6)
    child = svc.add_task(title="Child-1", description=None, start_date=None, deadline=None, priority=4, parent_id=parent.id)
...
def test_add_multiple_children_under_same_parent():
    ...
    parent = svc.add_task(title="Parent-X", description=None, start_date=None, deadline=None, priority=5)
    c1 = svc.add_task(title="C1", description=None, start_date=None, deadline=None, priority=4, parent_id=parent.id)
    c2 = svc.add_task(title="C2", description=None, start_date=None, deadline=None, priority=7, parent_id=parent.id)
...
def test_add_task_with_inactive_parent_raises_value_error():
    ...
    parent = svc.add_task(title="P", description=None, start_date=None, deadline=None, priority=5)
    svc.archive_task(parent.id)  # parent.active -> False
    with pytest.raises(ValueError) as exc:
        svc.add_task(title="C", description=None, start_date=None, deadline=None, priority=5, parent_id=parent.id)
...
def test_add_task_with_invalid_priority_raises_value_error(bad_bucket: int):
    ...
    with pytest.raises(ValueError) as exc:
        svc.add_task(
            ...
            start_date=None,
            deadline=None,
            priority=bad_bucket,  # out of range
        )
