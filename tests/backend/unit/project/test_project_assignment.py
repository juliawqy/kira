...
from tests.mock_data.project_data import MANAGER_USER, ASSIGNABLE_USER, DUPLICATE_USER, NOT_FOUND_ID

# UNI-081/001
@patch("backend.src.services.project.SessionLocal")
def test_assign_user_to_project_success(mock_session_local):
    mock_session = MagicMock()
    mock_session.get.side_effect = lambda model, id: MagicMock() if id in [1, 3] else None  # project_id=1, user_id=3
    mock_session.query().filter_by().first.return_value = None  # no existing assignment

    mock_session_local.begin.return_value.__enter__.return_value = mock_session

    result = project_service.assign_user_to_project(1, ASSIGNABLE_USER["user_id"])
...
def test_assign_nonexistent_user_to_project(mock_session_local):
    ...
    with pytest.raises(ValueError, match="User not found"):
        project_service.assign_user_to_project(1, NOT_FOUND_ID)
...
def test_duplicate_assignment_raises(mock_session_local):
    ...
    with pytest.raises(ValueError, match="already assigned"):
        project_service.assign_user_to_project(1, DUPLICATE_USER["user_id"])
