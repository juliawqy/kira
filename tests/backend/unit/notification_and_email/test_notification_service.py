"""Unit tests for NotificationService using centralized mock data fixtures."""
import pytest
...
class TestNotificationService:
    ...
    def mock_email_service(self):
        m = Mock()
        # Ensure recipients are resolved to trigger send_email path
    ...
    def test_notify_task_updated_success(self, notification_service, mock_email_service, notification_valid_update):
        ...
        result = notification_service.notify_task_updated(
            ...
            task_url=notification_valid_update.get("task_url"),
        )
    ...
    def test_notify_task_updated_with_all_parameters(self, notification_service, mock_email_service, notification_multiple_fields_update):
        ...
        result = notification_service.notify_task_updated(
            ...
            task_url=notification_multiple_fields_update.get("task_url"),
        )
    ...
    def test_logging_on_success(self, mock_logger, notification_service, mock_email_service, notification_minimal_update):
        ...
        mock_logger.info.assert_any_call(f"Activity notification 'task_update' sent for task {notification_minimal_update['task_id']}")
    ...
    def test_logging_on_email_failure(self, mock_logger, notification_service, mock_email_service, notification_minimal_update):
        ...
        mock_logger.error.assert_called_with("Failed to send activity notification 'task_update': SMTP error")
    ...
    # --- Direct notify_activity coverage ---
    @patch('backend.src.services.notification_service.logger')
    # UNI-124/055
    def test_notify_activity_success_logs(self, mock_logger, mock_email_service, notify_activity_success_params):
        ...
        with patch('backend.src.services.notification_service.get_email_service', return_value=mock_email_service):
            ...
            mock_logger.info.assert_any_call(
                ...
                extra={'type': 'task_update', 'task_id': notify_activity_success_params['task_id'], 'recipients_count': 1, 'initiated_by': notify_activity_success_params['user_email']}
            )
            mock_logger.info.assert_any_call(f"Activity notification 'task_update' sent for task {notify_activity_success_params['task_id']}")
    ...
    def test_notify_activity_failure_logs(self, mock_logger, mock_email_service, notify_activity_failure_params):
        ...
        with patch('backend.src.services.notification_service.get_email_service', return_value=mock_email_service):
            ...
            mock_logger.error.assert_called_with("Failed to send activity notification 'task_update': Boom")
    ...
    def test_notify_task_updated_exception_handler_branch(self, notification_minimal_update):
        ...
        with patch('backend.src.services.notification_service.get_email_service') as mock_get:
    ...
    def test_notify_task_updated_with_none_values(self, notification_null_values):
        ...
        with patch('backend.src.services.notification_service.get_email_service', return_value=mock_email_service):
            ...
            result = svc.notify_task_updated(
                ...
                task_url=notification_null_values.get("task_url"),
            )
            assert mock_email_service.send_email.call_count == 1
            assert result.success is True
    ...
    def test_notify_task_updated_large_data(self, notification_large_update):
        ...
        with patch('backend.src.services.notification_service.get_email_service', return_value=mock_email_service):
            ...
            result = svc.notify_task_updated(
                ...
                task_url=notification_large_update.get("task_url"),
            )
            mock_email_service.send_email.assert_called_once()
            assert result.success is True
